...
function api_funnels_lead_update($row) {
    ...
    if ($row['ok'] && !empty($row['data'])) {

            $rowUpd = array();

            $rowUpd['crm_member_id'] = $row['data']['crm_member_id'];
            $rowUpd['crm_lead_id'] = $row['data']['crm_lead_id'];

            $rowUpd['id'] = $row['data']['lead_id'];
            $rowUpd['status'] = 1;
            $rowUpd['status_info'] = $row["msg"] ? $row["msg"] : 'CREAT';
            $rowUpd['published'] = 0;
            
            $result = db_update_row(TABLE_FUNNEL_LEADS, $rowUpd);

            if (!empty($rowUpd['crm_member_id'])) {
                $rowUpdUser = array();
                $rowUpdUser['crm_id'] = $rowUpd['crm_member_id'];
                $rowUpdUser['id'] = $row['data']['member_user_id'];

                $result = db_update_row(TABLE_USERS, $rowUpdUser);
            }
